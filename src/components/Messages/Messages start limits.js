...
class Messages extends Component {
    ...
    componentDidMount() {
        /* this.unsubscribe = db.collection(`teams/#codewithsimran/channels/#general/posts`)
            ...
            //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            //********** there will probably be a problem with the realtime listeners when a document gets added, but what if I instead use the last query for a starting poit for the next(probably there will still be the same problem coz afterit gets it the first time for the next listeners, they ill remain listening to the first variant of the old query and won't get refreshed everytime the first query changes) => that most probably won't change anything
            ...
            .onSnapshot((querySnapshot) => {
                ...
                querySnapshot.docChanges().forEach((change, index) => {
                    ...
                    if (change.type === "modified") {
                        const docIndex = messages.findIndex((document) => document.key === id);

                        messages[docIndex] = doc;
                    }
                    if (change.type === "removed") { console.log('removed????'); }

                    if (index === querySnapshot.docs.length - 1 && addedCount === querySnapshot.docs.length) this.lastVisible = querySnapshot.docs[querySnapshot.docs.length - 1];

                });

                if (addedCount === querySnapshot.docs.length) messages.reverse();
